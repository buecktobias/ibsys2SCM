...
def test_add_item_duplicate_warning(caplog):
    ...
    caplog.clear()
    builder.add_item("E1")
    initial_count = len(builder.graph.nodes)
    builder.add_item("E1")  # Duplicate addition
    assert len(builder.graph.nodes) == initial_count
...
def test_build_cycle_raises():
    ...
    builder.graph.add_edge("E1", "E2")
    builder.graph.add_edge("E2", "E1")
...
def test_convert_input_dict():
    ...
    builder.add_item("E1")
    builder.add_item("E2")
    subgraph = builder.create_group("group1")
    inputs = {"E1": 2, "E2": 3}
...
def test_builder_subgraph_add_process_edges():
    ...
    # Find the process node added to the graph
    process_node_uid = None
    for uid, attr in builder.graph.nodes(data=True):
    ...
    assert builder.graph.has_edge(input_uid, process_node_uid)
    # Verify edge from the process to output item E2 with weight 1
    output_uid = builder.material_map["E2"].node_uid
    assert builder.graph.has_edge(process_node_uid, output_uid)
    assert builder.graph.edges[process_node_uid, output_uid]["weight"] == 1


def test_material_product_flow_graph_add_operator():
    # Create first graph with one node.
    g1 = nx.DiGraph()
    g1.add_node("A", data="node_A")
    mpg1 = MaterialProductFlowGraph(g1)
    # Create second graph with a different node.
    g2 = nx.DiGraph()
    g2.add_node("B", data="node_B")
    mpg2 = MaterialProductFlowGraph(g2)
    combined = mpg1 + mpg2
    nodes = list(combined.graph.nodes)
    assert "A" in nodes
    assert "B" in nodes
